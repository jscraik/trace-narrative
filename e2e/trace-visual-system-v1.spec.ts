...
async function loadPerfFixture(): Promise<TracePerfFixture> {
  const fixturePath = new URL('./fixtures/trace-large-timeline.json', import.meta.url);
...
test.describe('Trace Visual System v1', () => {
  ...
  test('surfaces toggle persistence failures in ImportErrorBanner when they occur', async ({ page }) => {
    ...
    const persistenceError = page.locator('text=Unable to persist Trace setting');
    const traceSignal = page.locator('[data-testid="trace-signal"]');
    ...
    await page.reload();
    const traceSignalAfterReload = page.locator('[data-testid="trace-signal"]');
    if (isDevRuntime) {
  ...
  test('@trace-perf captures frame metrics and writes verification artifact', async ({ page, browserName }) => {
          ...
          const observer = new PerformanceObserver((list) => {
            for (const entry of list.getEntries()) {
              ...
              if (!shift.hadRecentInput && typeof shift.value === 'number') {
                const traceRelated = (shift.sources ?? []).some((source) => {
                  const node = source.node;
                  return node instanceof Element && Boolean(node.closest('[data-testid="trace-signal"]'));
                });
    ...
    if (!perfResult) {
      throw new Error('Failed to capture Trace perf metrics after retry attempts.');
    }
    ...
    const outputPath = path.join(artifactRoot, `trace-perf-${dateStamp}.json`);
    const docsArtifactPath = process.env.TRACE_PERF_WRITE_DOCS_ARTIFACT === '1'
      ? path.join(process.cwd(), 'docs', 'assets', 'verification', `trace-perf-${dateStamp}.json`)
      : null;
