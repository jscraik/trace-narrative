...
import { cacheCommitSummaries, upsertRepo } from "../db";
import {
...
describe("Indexer Performance (Cold Latency Benchmark)", () => {
	beforeEach(() => {
		...
		mockResolveGitRoot.mockImplementation(async () => {
			await delay(5);
			return "/test/perf/repo";
		});
		mockGetHeadBranch.mockImplementation(async () => {
			await delay(5);
			return "main";
		});
		mockGetHeadSha.mockImplementation(async () => {
			await delay(5);
			return "head123";
		});
		mockUpsertRepo.mockImplementation(async () => {
			await delay(5);
			return 1;
		});
		mockListCommits.mockImplementation(async () => {
			await delay(20);
			return mockCommits;
		});
		mockCacheCommitSummaries.mockResolvedValue(undefined);
		mockGetAggregateStatsForCommits.mockImplementation(async () => {
			await delay(10);
			return { added: 100, removed: 50, uniqueFiles: 10 };
		});

		// Simulate slow Tauri commands mapped to disk reads / git queries (50ms base delay each)
		mockGetDirtyFiles.mockImplementation(async () => {
			await delay(50);
			return [];
		});
		mockGetWorkingTreeChurn.mockImplementation(async () => {
			await delay(50);
			return 0;
		});
		mockImportAttributionNotesBatch.mockImplementation(async () => {
			await delay(50);
			return { total: 0, imported: 0, missing: 0, failed: 0 };
		});
		mockImportSessionLinkNotesBatch.mockImplementation(async () => {
			await delay(50);
			return { total: 0, imported: 0, missing: 0, failed: 0 };
		});
		mockGetStoryAnchorStatus.mockImplementation(async () => {
			await delay(50);
			return [];
		});
		mockLoadSessionExcerpts.mockImplementation(async () => {
			await delay(50);
			return [];
		});
		mockLoadTraceConfig.mockImplementation(async () => {
			await delay(50);
			return {
				codexOtelLogPath: "/tmp/otel.log",
				codexOtelReceiverEnabled: false,
			};
		});
		mockIngestCodexOtelLogFile.mockImplementation(async () => {
			await delay(100);
			return {
				status: { state: "inactive", message: "None" },
				recordsWritten: 0,
			};
		});
		mockScanAgentTraceRecords.mockImplementation(async () => {
			await delay(50);
			return {
				byCommit: {},
				byFileByCommit: {},
		...
		mockGetLatestTestRunSummaryByCommit.mockImplementation(async () => {
			await delay(50);
			return {};
		});
