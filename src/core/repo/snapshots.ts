...
import { listNarrativeFiles, readNarrativeFile, writeNarrativeFile, ensureNarrativeDirs } from '../tauri/narrativeFs';
import { getDirtyFiles, getHeadBranch, getHeadSha, getWorkingTreeChurn } from './git';
...
export async function captureSnapshot(
  ...
  await writeNarrativeFile(repoRoot, filename, JSON.stringify(snapshot, null, 2));
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.warn(`[Snapshots] Failed to read snapshot ${file}:`, e);
      }
    }
    
    // Sort by date descending
    return snapshots.sort((a, b) => new Date(b.atISO).getTime() - new Date(a.atISO).getTime());
  } catch (_e) {
...
export async function autoCaptureIfNeeded(repoRoot: string): Promise<AutoCaptureResult> {
  ...
  if (dirtyFiles.length === 0) return { snapshot: null, dirtyFiles, dirtyChurnLines };
  ...
  if (lastAuto) {
    ...
    const sameFiles = JSON.stringify(lastFiles) === JSON.stringify(currentFiles);
