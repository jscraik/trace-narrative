...
describe("setIngestConfig", () => {
	...
	it("propagates errors from the backend", async () => {
		...
		await expect(setIngestConfig({ retentionDays: -1 })).rejects.toThrow(
			"validation failed",
		);
	});
});
...
describe("backfillRecentSessions", () => {
	...
	it("uses default limitPerTool of 10 when omitted", async () => {
		...
		const [, args] = mockInvoke.mock.calls[0] as [
			string,
			{ limitPerTool: number },
		];
...
describe("codexAppServerSubmitApproval", () => {
	it("sends all required approval fields", async () => {
		...
		await codexAppServerSubmitApproval(
			"req-1",
			"thread-1",
			"token-abc",
			true,
...
describe("configureCodexOtel — URL validation", () => {
	...
	it("throws for a javascript: protocol URL (not http/https)", async () => {
		await expect(configureCodexOtel("javascript:alert(1)")).rejects.toThrow(
	...
	it("throws for a file: URL", async () => {
		await expect(configureCodexOtel("file:///etc/passwd")).rejects.toThrow(
			"http or https protocol",
		);
		expect(mockInvoke).not.toHaveBeenCalled();
	});
	...
	it("throws for an empty string", async () => {
		await expect(configureCodexOtel("")).rejects.toThrow(
			"Invalid endpoint URL",
		);
		expect(mockInvoke).not.toHaveBeenCalled();
	});
	...
	it("propagates backend errors for valid URLs", async () => {
		...
		await expect(configureCodexOtel("http://localhost:4317")).rejects.toThrow(
			"connection refused",
		);
	});
});
