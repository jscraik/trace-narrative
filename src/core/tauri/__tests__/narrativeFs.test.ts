...
describe("ensureNarrativeDirs", () => {
	...
	it("propagates errors from the Tauri backend", async () => {
		...
		await expect(ensureNarrativeDirs("/locked")).rejects.toThrow(
			"permission denied",
		);
	});
});
...
describe("listNarrativeFiles", () => {
	...
	it("propagates errors from the backend", async () => {
		...
		await expect(listNarrativeFiles("/repo", "bad-dir")).rejects.toThrow(
			"directory not found",
		);
	});
});
...
describe("readTextFile", () => {
	...
	it("propagates errors for non-existent paths", async () => {
		...
		await expect(readTextFile("/not/there.json")).rejects.toThrow(
			"path does not exist",
		);
	});
});
...
describe("fileExists", () => {
	...
	it("propagates unexpected errors (e.g. IPC timeout)", async () => {
		...
		await expect(fileExists("/repo", "src/a.ts")).rejects.toThrow(
			"IPC timeout",
		);
	});
});
