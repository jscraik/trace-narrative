...
export function generateQueryId(input: AskWhyQuestionInput): string {
	...
	return simpleHash(
		`${input.repoId}:${input.branchId}:${input.question}:${minuteBucket}`,
	);
}
...
const CITATION_TYPE_WEIGHT: Record<string, number> = {
	commit: 3,
	session: 2,
	file: 1,
	diff: 0,
...
export function convertEvidenceLinks(
...
export function rankAndCapCitations(
	...
	return [...citations]
		.sort(
			(a, b) =>
				(CITATION_TYPE_WEIGHT[b.type] ?? 0) -
					(CITATION_TYPE_WEIGHT[a.type] ?? 0) || a.id.localeCompare(b.id),
		)
		.slice(0, ASK_WHY_CITATION_CAP);
}

// Sentence Citation Map (Phase 1: single sentence)
export function buildSentenceCitationMap(
	citations: AskWhyCitation[],
): AskWhySentenceCitationMapEntry[] {
	const citationIds = citations.map((c) => c.id);
	return [
		{ sentenceIndex: 0, citationIds, uncertain: citationIds.length === 0 },
	];
}
...
export async function composeAskWhyAnswer(
	...
	const questionHash =
		precomputed?.questionHash ?? (await hashQuestion(input.question));
	...
	const citations = rankAndCapCitations(
		convertEvidenceLinks(narrative.evidenceLinks ?? []),
	);
	const confidence = Math.max(0, Math.min(1, narrative.confidence ?? 0.5));
	const confidenceBand = classifyConfidenceBand(confidence);
	const hasEvidence =
		narrative.evidenceLinks && narrative.evidenceLinks.length > 0;
	...
	if (confidenceBand === "low" || !hasEvidence) {
		const reasonCode: AskWhyFallbackReasonCode = !hasEvidence
			? "no_evidence"
			: "low_confidence_override";
		return {
			kind: "fallback",
			answer: { ...answer, fallbackUsed: true, fallbackReasonCode: reasonCode },
			reasonCode,
		};
	}
