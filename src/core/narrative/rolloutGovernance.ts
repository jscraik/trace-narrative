...
export function evaluateNarrativeRollout(args: {
	...
	const projectionCompletenessScore = (
		["executive", "manager", "engineer"] as const
	).reduce((score, key) => {
	...
	const projectionMetric = toMetric({
		...
		threshold: 0.7,
		rationale:
			"Executive/Manager/Engineer projections should each include headline, bullets, and evidence links.",
	});
	...
	const connectorMetric = toMetric({
		...
		rationale:
			githubContextState.status === "error"
				? (githubContextState.error ??
					"GitHub connector reported an ingestion error.")
				: githubContextState.status === "partial"
					? (githubContextState.error ??
						"GitHub connector loaded partially with recoverable errors.")
	...
	const rules = [
		...
		},
		{
			...
			severity: "critical" as const,
			triggered:
				narrative.confidence < 0.45 && narrative.evidenceLinks.length < 2,
			rationale:
				"Low-confidence summaries without enough evidence are unsafe to present as primary output.",
		},
		{
			id: "connector_error",
			...
			triggered:
				...
				githubContextState.status === "partial",
			rationale:
				"Connector errors should degrade confidence and be reviewed before rollout expansion.",
		},
		{
	...
	const averageScore = roundScore(
		rubric.reduce((sum, metric) => sum + metric.score, 0) /
	...
	const hasWarning = rules.some(
		(rule) => rule.triggered && rule.severity === "warning",
	);
	...
	return {
		status:
			hasCritical || hasRubricFail
				? "rollback"
				: hasWarning || hasRubricWarn
					? "watch"
					: "healthy",
		rubric,
		rules,
