...
vi.mock("../../components/dashboard/DashboardEmptyState", () => ({
	DashboardEmptyState: ({ reason }: { reason: string }) => (
		<div data-testid="dashboard-empty">{reason}</div>
	),
}));
...
vi.mock("../../components/dashboard/DashboardHeader", () => ({
	DashboardHeader: ({
		...
		<div
			...
			data-trust-state={trustState ?? "healthy"}
		>
			{repoName}
		</div>
	),
...
describe("DashboardView", () => {
	...
	it("surfaces degraded trust state in the dashboard header", async () => {
		mockGetDashboardStats.mockResolvedValue(
			createDashboardStats(1, "repo-one"),
		);
	...
	it("treats permission denied failures as non-retryable", async () => {
		mockGetDashboardStats.mockRejectedValue(
			new Error("Permission denied for dashboard command"),
		);
		const setActionError = vi.fn();
		...
		expect(setActionError).toHaveBeenCalledWith(
			"Permission denied for dashboard command",
		);
	});
