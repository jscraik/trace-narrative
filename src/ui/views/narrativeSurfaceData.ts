...
function mapDriftStatusToActivityStatus(
...
const surfaceDefinitions: Record<SurfaceMode, SurfaceDefinition> = {
	...
	assistant: {
		...
		title: "Codex Copilot",
		subtitle: () =>
			"A Codex-first copilot for repos, sessions, costs, and hygiene.",
		heroTitle: (context) =>
			`Turn ${context.repoName} into a Codex-grounded narrative partner.`,
		heroBody: () =>
		...
		metrics: (context) => [
			{
				label: "Context packs",
				value: "6",
				detail: "Repo, session, cost, hooks, hygiene, docs",
				tone: "blue",
			},
			{
				...
				value: "Codex-first",
				detail:
					"Additional providers come after the core shell feels trustworthy",
				tone: "violet",
			},
			{
				label: "Action guards",
				value: "Fail-closed",
				detail: "Recommend before acting",
				tone: "green",
			},
			{
				label: "Drift alert",
				value: context.driftReport
					? context.driftReport.status === "healthy"
						? "Healthy"
						: "Drifting"
					: "Calculating",
				detail:
					context.driftReport?.metrics[1].rationale ||
					"Analyzing workspace churn...",
				tone: !context.driftReport
					? "slate"
					: context.driftReport.status === "healthy"
						? "green"
			...
			{
				label: "Repo scope",
				value: context.repoName,
				detail: "Primary active workspace",
				tone: "slate",
			},
		],
		...
		highlights: (context) => [
			...
			},
			{
				...
				body: "Use trust posture, workspace health, and recent activity to recommend the next safe move, not just the loudest move.",
				tone: "blue",
			},
			{
				eyebrow: "Protect",
				title: "What is risky to clean up?",
				body: "Make cleanup suggestions explicit about blast radius, rollback posture, and hidden dependencies before anything looks routine.",
				tone: "amber",
			},
		],
		...
		activity: (context) => [
			...context.sessionExcerpts.slice(0, 2).map((s) => ({
				...
				action: s.linkedCommitSha
					? { type: "open_raw_diff" as const, commitSha: s.linkedCommitSha }
					: undefined,
			})),
			...
			...(context.driftReport && context.driftReport.status !== "healthy"
				? [
						{
							...
							meta: "System",
							detail:
								context.driftReport.metrics.find(
									(m) => m.status === "critical" || m.status === "warn",
								)?.rationale || "Workspace is drifting from narrative.",
							status: mapDriftStatusToActivityStatus(
								context.driftReport.status,
							),
				...
				: []),
			{
				title: "Ask for the branch story",
				meta: "Prompt",
				detail: `Summarize the biggest deltas in ${context.repoName} and explain the evidence behind them.`,
		...
		tableRows: () => [
			{
				primary: "Repo narrative",
				secondary: "Ground truth",
				tertiary: "Explains what changed and why",
			},
			{
				primary: "Session excerpts",
				secondary: "Operator memory",
				tertiary: "Adds model and tool context to changes",
			},
			{
				primary: "Dashboard stats",
				secondary: "Signal layer",
				tertiary: "Summarizes activity, costs, and attribution",
			},
			{
				primary: "Health checks",
				secondary: "Guardrail layer",
				tertiary: "Prevents confident but unsafe suggestions",
			},
		],
		footerNote: () =>
			"Recommendation: keep the assistant opinionated and context-rich, not an unbounded general chat surface.",
	},
	live: {
		section: "Evidence",
		title: "Live",
	...
	snapshots: {
		...
		title: "Snapshots",
		subtitle: () =>
			"Saved workspace states, branch snapshots, and recovery moments.",
		heroTitle: () =>
			"Treat workspace state as something you can revisit, not just remember.",
		heroBody: () =>
		...
		metrics: (context) => [
			{
				...
				value: String(context.snapshots.length),
				detail: `${context.snapshots.length} snapshots in local storage`,
				tone: "blue",
			},
			{
				label: "Repo state",
				value: context.hasLiveRepoData ? "Live" : "Static",
				detail: "Based on current loader health",
				tone: "violet",
			},
			{
				label: "Recovery ready",
				value: context.snapshots.length > 0 ? "Yes" : "No",
				detail:
					context.snapshots.length > 0
						? "Latest snapshots available"
						: "Awaiting local capture service",
				tone: "green",
			},
			{
				label: "Drift staleness",
				value: `${context.driftReport?.metrics.find((m) => m.id === "snapshot_staleness")?.value || 0}h`,
				detail: "Time since last snapshot",
				tone: context.driftReport?.status === "healthy" ? "green" : "amber",
			},
			{
				label: "Files in dirty state",
				value: String(context.changedFiles.length),
				detail: "Uncommitted changes in workspace",
				tone: "amber",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Snapshot",
				title: "Freeze an operator moment",
				body: "Capture repo state, session context, and intended next move together.",
				tone: "blue",
			},
			{
				eyebrow: "Compare",
				title: "Detect drift from plan",
				body: "Use snapshots to spot how far the live workspace has moved from the last verified baseline.",
				tone: "amber",
			},
			{
				eyebrow: "Recover",
				title: "Support rollback reasoning",
				body: "Keep enough metadata that a snapshot can justify a revert or a retry, not just exist as a timestamp.",
				tone: "violet",
			},
		],
					...
					? [
							{
								...
								meta: "Idle",
								detail: "Snapshot engine is active but awaiting trigger",
								status: "info",
							},
						]
					: [],
		...
		footerNote: () =>
			"Snapshots become powerful when they explain what was true, not just when they were saved.",
	},
	worktrees: {
		section: "Workspace",
		title: "Worktrees",
	...
	attribution: {
		...
		title: "Attribution",
		subtitle: () =>
			"AI vs human contribution signals, evidence gaps, and trust cues.",
		...
		metrics: (context) => [
			{
				label: "AI share",
				value: "68%",
				detail: "Across the current analysis window",
				tone: "violet",
			},
			{
				label: "Unknown lines",
				value: "11%",
				detail: "Need more evidence or better linking",
				tone: "amber",
			},
			{
				label: "Human-led files",
				value: "7",
				detail: "Keep visible to avoid flattening all work into AI",
				tone: "blue",
			},
			{
				label: "Trust state",
				value: context.trustState === "healthy" ? "Healthy" : "Caution",
				detail: context.trustLabel,
				tone: context.trustState === "healthy" ? "green" : "amber",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Evidence",
				title: "Confidence beats certainty theater",
				body: "If a line cannot be attributed strongly, the UI should say so plainly and offer the next-best evidence path.",
				tone: "amber",
			},
			{
				eyebrow: "Comparison",
				title: "Keep the human contribution visible",
				body: "Avoid turning attribution into an AI-only lens; the point is balance and explanation.",
				tone: "blue",
			},
			{
				eyebrow: "Reviewability",
				title: "File-level hotspots matter",
				body: "Surface files where attribution volatility is highest so the operator knows where to inspect deeper.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "Unknown-only segments found",
				meta: "Today",
				detail:
					"A few files still lack enough signal for confident attribution",
				status: "warn",
			},
			{
				title: "High-confidence AI lane",
				meta: "Today",
				detail:
					"Dashboard and telemetry changes have strong agent-session evidence",
				status: "ok",
			},
			{
				title: "Human-only maintenance",
				meta: "Today",
				detail: "Some cleanup and repo hygiene remains intentionally human-led",
				status: "info",
			},
			{
				title: "Degraded capture overlay",
				meta: "When active",
				detail:
					"Any reliability drop should immediately soften attribution certainty",
				status: "critical",
			},
		],
		...
		tableRows: () => [
			{
				...
				secondary: "Mixed authorship",
				tertiary: "Inspect session-linked file changes",
			},
			{
				primary: "Telemetry paths",
				secondary: "Confidence sensitive",
				tertiary: "Check degraded-capture warnings first",
			},
			{
				primary: "Styles and tokens",
				secondary: "Collaborative edits",
				tertiary: "Compare design-system intent vs live diffs",
			},
			{
				primary: "Docs artifacts",
				secondary: "Historical drift",
				tertiary: "Use narrative evidence before re-labeling authorship",
			},
		],
		footerNote: () =>
			"Attribution page should reinforce trust, not weaken it by overclaiming.",
	},
	skills: {
		section: "Integrations",
		title: "Skills",
		...
		metrics: () => [
			{
				...
				value: "82",
				detail: "Across shared and repo-specific paths",
				tone: "blue",
			},
			{
				label: "Recently used",
				value: "14",
				detail: "Touched in the last day",
				tone: "violet",
			},
			{
				label: "Missing metadata",
				value: "3",
				detail: "Need cleanup for better discoverability",
				tone: "amber",
			},
			{
				label: "Healthy paths",
				value: "Mostly",
				detail: "Shared skill registry resolving correctly",
				tone: "green",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Discover",
				title: "Find the right workflow fast",
				body: "Make it obvious which skill helps with docs, UI, telemetry, or workflow routing.",
				tone: "blue",
			},
			{
				eyebrow: "Trust",
				title: "Expose stale or missing paths",
				body: "Users should see path health and last-touch signals, not just skill names.",
				tone: "amber",
			},
			{
				eyebrow: "Reuse",
				title: "Support operator habits",
				body: "Promote the skills that turn repeated work into consistent, auditable flows.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				...
				meta: "active",
				detail:
					"Used to define the new surface direction before implementation",
				status: "ok",
			},
			{
				title: "diagram-cli",
				meta: "recent",
				detail: "Helpful for architecture-first routing before UI work",
				status: "info",
			},
			{
				title: "systematic-debugging",
				meta: "idle",
				detail: "Best reserved for failures rather than speculative use",
				status: "info",
			},
			{
				title: "ui-visual-regression",
				meta: "next",
				detail: "A good follow-up once visual parity and polish work deepens",
				status: "warn",
			},
		],
		...
		tableRows: () => [
			{
				...
				secondary: "Scope decisions",
				tertiary: "Use before committing to major UI structure",
			},
			{
				primary: "diagram-cli",
				secondary: "Architecture bootstrap",
				tertiary: "Required for repo-first understanding",
			},
			{
				primary: "baseline-ui",
				secondary: "Visual consistency",
				tertiary: "Good follow-up for polish pass",
			},
			{
				primary: "playwright-interactive",
				secondary: "Visual QA",
				tertiary: "Useful once surface routes render live",
			},
		],
		footerNote: () =>
			"Skills view should help the operator choose the right workflow, not drown them in catalogue depth.",
	},
	agents: {
		section: "Integrations",
		title: "Agents",
		subtitle: () =>
			"Repo-local agent definitions, multi-agent roles, and orchestration readiness.",
		heroTitle: () =>
			"Make agent surfaces explicit when they exist, and honest when they do not.",
		heroBody: () =>
		...
		metrics: () => [
			{
				...
				value: "5",
				detail: "Across shared and repo-local configs",
				tone: "blue",
			},
			{
				label: "Ready roles",
				value: "3",
				detail: "Useful now for review or execution",
				tone: "green",
			},
			{
				label: "Gaps",
				value: "2",
				detail: "Need clearer ownership or prompts",
				tone: "amber",
			},
			{
				label: "Parallel lanes",
				value: "Available",
				detail: "Use only when work is genuinely disjoint",
				tone: "violet",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Presence",
				title: "Show real agent availability",
				body: "If no repo-local agents exist, prefer a meaningful empty state over fake completeness.",
				tone: "blue",
			},
			{
				eyebrow: "Readiness",
				title: "Clarify what each role is for",
				body: "Help the operator decide whether to keep work local or fan out.",
				tone: "violet",
			},
			{
				eyebrow: "Safety",
				title: "Avoid parallelism theater",
				body: "Surface that multi-agent work is useful only when writes and ownership are truly separate.",
				tone: "amber",
			},
		],
		...
		activity: () => [
			{
				title: "Reviewer lane available",
				meta: "Now",
				detail: "Useful for post-implementation simplicity or parity review",
				status: "ok",
			},
			{
				title: "UI worker optional",
				meta: "Now",
				detail: "Good fit when visual slices have disjoint write scopes",
				status: "info",
			},
			{
				title: "Critical path local",
				meta: "Now",
				detail:
					"Routing and shell integration work should stay in the main flow",
				status: "warn",
			},
			{
				title: "Context handoff cost",
				meta: "Always",
				detail:
					"Only fan out when the saved time outweighs the briefing overhead",
				status: "critical",
			},
		],
		...
		tableRows: () => [
			{
				primary: "worker",
				secondary: "Bounded implementation task",
				tertiary: "Good for isolated component work",
			},
			{
				primary: "design-implementation-reviewer",
				secondary: "Visual discrepancy review",
				tertiary: "Useful after the first surface pass lands",
			},
			{
				primary: "kieran-typescript-reviewer",
				secondary: "Strict TS review",
				tertiary: "Helpful after surface data wiring",
			},
			{
				primary: "monitor",
				secondary: "Long-running poll",
				tertiary: "Only for external waits, not this UI pass",
			},
		],
		footerNote: () =>
			"Agents page should help the operator decide when not to delegate just as much as when to do it.",
	},
	memory: {
		section: "Integrations",
		title: "Memory",
		subtitle: () =>
			"Workspace memory state, reusable learnings, and context hygiene.",
		...
		metrics: () => [
			{
				...
				value: "6",
				detail: "Directly related to this repo and task",
				tone: "blue",
			},
			{
				label: "Potential drift",
				value: "1",
				detail: "Naming/path corrections recently required",
				tone: "amber",
			},
			{
				label: "Reusable lanes",
				value: "3",
				detail: "Dashboard, telemetry, trust loops",
				tone: "violet",
			},
			{
				label: "Context hygiene",
				value: "Good",
				detail: "Memory stays bounded and cited",
				tone: "green",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Use lightly",
				title: "Pull memory only when it materially helps",
				body: "Avoid bloating UI or implementation passes with irrelevant historical detail.",
				tone: "blue",
			},
			{
				eyebrow: "Verify",
				title: "Trust current evidence over stale notes",
				body: "If repo state contradicts memory, update memory and proceed with the live truth.",
				tone: "amber",
			},
			{
				eyebrow: "Cite",
				title: "Keep reuse auditable",
				body: "Memory should explain why a direction was chosen, not silently dictate it.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "Dashboard v3 contract lane",
				meta: "Relevant",
				detail: "Useful for trust-state and workflow-spec routing guidance",
				status: "ok",
			},
			{
				title: "Repo naming drift",
				meta: "Resolved",
				detail:
					"trace-narrative is the active worktree; older names are historical",
				status: "warn",
			},
			{
				title: "Telemetry hardening notes",
				meta: "Relevant",
				detail:
					"Support reliability messaging across shared narrative surfaces",
				status: "info",
			},
			{
				title: "Unverified specifics",
				meta: "Avoid",
				detail:
					"Do not present older counts or paths as current without checking",
				status: "critical",
			},
		],
		...
		tableRows: () => [
			{
				primary: "dashboard_v3_contract_phase",
				secondary: "High",
				tertiary: "Guide view semantics and trust handling",
			},
			{
				primary: "trace-narrative naming drift",
				secondary: "High",
				tertiary: "Avoid stale historical workspace labels in UI copy",
			},
			{
				primary: "telemetry agents hardening",
				secondary: "Medium",
				tertiary: "Inform live and status view messaging",
			},
			{
				primary: "older product-roadmap notes",
				secondary: "Low",
				tertiary: "Use only if they still match repo evidence",
			},
		],
		footerNote: () =>
			"Memory page should keep prior context useful without letting it outrank current repository truth.",
	},
	hooks: {
		section: "Integrations",
		title: "Hooks",
		subtitle: () =>
			"Trigger points, health, and command safety around workstation hooks.",
		...
		metrics: () => [
			{
				label: "Hook count",
				value: "9",
				detail: "Across relevant repos and tools",
				tone: "blue",
			},
			{
				label: "Passing",
				value: "7",
				detail: "Observed healthy on recent runs",
				tone: "green",
			},
			{
				label: "Flaky",
				value: "2",
				detail: "Need closer inspection",
				tone: "amber",
			},
			{
				label: "Risk posture",
				value: "Visible",
				detail: "Blast radius called out before action",
				tone: "violet",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Visibility",
				title: "Show trigger stage and command",
				body: "Operators need to know what will fire before they kick off a workflow.",
				tone: "blue",
			},
			{
				eyebrow: "Safety",
				title: "Flag risky hooks early",
				body: "Especially where hooks can block pushes, mutate files, or rely on fragile env state.",
				tone: "amber",
			},
			{
				eyebrow: "Repair",
				title: "Point to the right follow-up",
				body: "A failing hook should lead to remediation guidance, not a dead-end red badge.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "pre-commit",
				meta: "stable",
				detail: "Lint and type checks remain predictable",
				status: "ok",
			},
			{
				title: "pre-push",
				meta: "heavy",
				detail: "Runs deep verification and can surface policy drift quickly",
				status: "warn",
			},
			{
				...
				meta: "watch",
				detail:
					"Useful, but should not hide shell fragility or quoting failures",
				status: "info",
			},
			{
				title: "unknown custom hook",
				meta: "review",
				detail: "Needs clearer documentation before trust can rise",
				status: "critical",
			},
		],
		...
		tableRows: () => [
			{
				primary: "pre-commit",
				secondary: "Lint + typecheck",
				tertiary: "Fast enough to keep enabled",
			},
			{
				primary: "pre-push",
				secondary: "Deep verification",
				tertiary: "Document heavy failure modes clearly",
			},
			{
				primary: "automation-webhook",
				secondary: "Background trigger",
				tertiary: "Make prompt safety visible",
			},
			{
				primary: "repo-local custom",
				secondary: "Unclear",
				tertiary: "Treat as caution until documented",
			},
		],
		footerNote: () =>
			"Hooks page should explain operational consequences, not just list file names.",
	},
	setup: {
		section: "Integrations",
		title: "Setup",
		subtitle: () =>
			"Repo instructions, config roots, MCP health, and onboarding readiness.",
		...
		metrics: () => [
			{
				label: "Instruction roots",
				value: "3",
				detail: "Global, repo, local docs",
				tone: "blue",
			},
			{
				label: "MCP endpoints",
				value: "Healthy",
				detail: "Core connectors responding",
				tone: "green",
			},
			{
				label: "Permissions",
				value: "Explicit",
				detail: "Least-privilege posture visible",
				tone: "violet",
			},
			{
				label: "Setup gaps",
				value: "2",
				detail: "Need follow-up before feature parity",
				tone: "amber",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Discovery",
				title: "Show where truth comes from",
				body: "Make AGENTS, instructions, and local setup files visible and explain their precedence.",
				tone: "blue",
			},
			{
				eyebrow: "Readiness",
				title: "Call out missing capabilities",
				body: "Users should know when setup gaps explain why a view is partial or empty.",
				tone: "amber",
			},
			{
				eyebrow: "Trust",
				title: "Protect the operator contract",
				body: "If a command boundary is missing, fail closed and say why.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "Architecture manifest present",
				meta: "Pass",
				detail: "Diagram artifacts were available before planning edits",
				status: "ok",
			},
			{
				title: "Learnings file read",
				meta: "Pass",
				detail: "Repo shell gotchas and tool patterns were loaded up front",
				status: "ok",
			},
			{
				title: "Config drift watch",
				meta: "Caution",
				detail: "Historical path drift still matters for docs and memory",
				status: "warn",
			},
			{
				title: "Capabilities review",
				meta: "Ongoing",
				detail: "Keep Tauri authority narrow as surface breadth grows",
				status: "info",
			},
		],
		...
		tableRows: () => [
			{
				primary: "AGENTS instructions",
				secondary: "Behavior contract",
				tertiary: "Loaded and guiding work",
			},
			{
				primary: "Architecture artifacts",
				secondary: "Routing context",
				tertiary: "Present and current enough for this pass",
			},
			{
				primary: "Tauri capabilities",
				secondary: "Authority boundary",
				tertiary: "Needs continued audit as views expand",
			},
			{
				primary: "Repo docs",
				secondary: "Human onboarding",
				tertiary: "Good foundation, still evolving",
			},
		],
		footerNote: () =>
			"Setup page should answer “why is this view behaving this way?” before the operator has to ask.",
	},
	ports: {
		section: "Integrations",
		title: "Ports",
		subtitle: () =>
			"Bound services, local addresses, and process ownership clues.",
		heroTitle: () =>
			"Local network state belongs in the workstation when it affects trust.",
		heroBody: () =>
		...
		metrics: () => [
			{
				label: "Bound ports",
				value: "5",
				detail: "Relevant to current tooling",
				tone: "blue",
			},
			{
				label: "Healthy listeners",
				value: "4",
				detail: "Responding as expected",
				tone: "green",
			},
			{
				label: "Unknown owners",
				value: "1",
		...
		highlights: () => [
			{
				eyebrow: "Health",
				title: "What is listening right now?",
				body: "Keep service ownership and purpose visible so troubleshooting starts from evidence.",
				tone: "blue",
			},
			{
				eyebrow: "Trust",
				title: "Unknown listeners should stand out",
				body: "Anything not clearly owned by the workspace deserves review, not a tiny footnote.",
				tone: "amber",
			},
			{
				eyebrow: "Context",
				title: "Tie ports back to features",
				body: "When a webhook or app server matters to the UI, point directly to the affected narrative surface.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "4318 / OTEL",
				meta: "active",
				detail: "Supports ingest and reliability visibility",
				status: "ok",
			},
			{
				title: "8787 / webhook",
				meta: "idle",
				detail: "Ready for agentation or local automation hooks",
				status: "info",
			},
			{
				title: "5173 / Vite dev",
				meta: "active",
				detail: "Primary frontend dev server while UI views evolve",
				status: "ok",
			},
			{
				title: "Unknown listener",
				meta: "review",
				detail: "Operator should confirm owner before trusting it",
				status: "critical",
			},
		],
		...
		tableRows: () => [
			{
				primary: "4318",
				secondary: "OTEL receiver",
				tertiary: "Feeds live and status reliability surfaces",
			},
			{
				primary: "5173",
				secondary: "Vite",
				tertiary: "Current UI development target",
			},
			{
				primary: "8787",
				secondary: "Webhook listener",
				tertiary: "Used for automation and assistant flows",
			},
			{
				primary: "3000",
				secondary: "Local app server",
				tertiary: "Potential assistant or MCP integration lane",
			},
		],
		footerNote: () =>
			"Ports page should stay diagnostic and quiet, not feel like a network operations console.",
	},
	hygiene: {
		section: "Health",
		title: "Hygiene",
		subtitle: () =>
			"Zombie processes, divergence, dead directories, and cleanup suggestions.",
		...
		metrics: (context) => [
			{
				label: "Stale sessions",
				value: String(Math.max(0, context.sessionCount - 5)),
				detail: "Candidates for cleanup or archival",
				tone: "amber",
			},
			{
				label: "Repo health",
				value: context.hasLiveRepoData ? "Stable" : "Unknown",
				detail: "Based on current index state",
				tone: "green",
			},
			{
				label: "Linked rate",
				value:
					context.sessionCount > 0
						? `${Math.round((context.sessionExcerpts.filter((s) => !!s.linkedCommitSha).length / context.sessionCount) * 100)}%`
						: "0%",
				detail: "Sessions with thread connections",
				tone: "blue",
			},
			{
				label: "Trust state",
				value: context.trustState === "healthy" ? "Verified" : "Review",
				detail: context.trustLabel,
				tone: "violet",
			},
		],
		...
		highlights: () => [
			{
				...
				body: "Present cleanup suggestions with explicit blast radius and rollback posture before any action is taken.",
				tone: "amber",
			},
			{
				eyebrow: "Auditability",
				title: "Explain why cleanup is recommended",
				body: "Tie every remediation suggestion to an observed state, not just a heuristic label.",
				tone: "blue",
			},
			{
				eyebrow: "Trust",
				title: "Human control stays visible",
				body: "The UI should never imply that silent cleanup is a normal background behavior.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "Stale branch worktree",
				meta: "High risk",
				detail:
					"Potential delete candidate blocked until merge state is confirmed",
				status: "critical",
			},
			{
				title: "Unpushed repo",
				meta: "Medium risk",
				detail: "May hide work that should be synced before cleanup",
				status: "warn",
			},
			{
				title: "Zombie process",
				meta: "Low risk",
				detail: "Safe to inspect, but still requires explicit operator choice",
				status: "info",
			},
			{
				title: "Dead directory",
				meta: "Low risk",
				detail: "Good candidate for guided cleanup once confirmed",
				status: "ok",
			},
		],
		...
		tableRows: (context) =>
			[
				...(context.sessionExcerpts.filter((s) => !s.linkedCommitSha).length > 0
					...
					: []),
				{
					primary: "Remote divergence",
					secondary: "Low",
					tertiary: "Review sync path and stash state first",
				},
				{
					primary: "Zombie process",
					secondary: "Low",
					tertiary: "Offer kill preview, not instant action",
				},
				{
					primary: "Dead directory",
					secondary: "Low",
					tertiary: "Surface path and owner before cleanup",
				},
			].slice(0, 4),
		footerNote: () =>
			"Hygiene page should feel trustworthy because it is cautious, not because it is aggressive.",
	},
	deps: {
		section: "Health",
		title: "Dependencies",
		...
		metrics: () => [
			{
				...
				value: "12",
				detail: "Across active repos",
				tone: "blue",
			},
			{
				label: "Major bumps",
				value: "4",
				detail: "Need deliberate review",
				tone: "amber",
			},
			{
				label: "Security flags",
				value: "1",
				detail: "Requires follow-up soon",
				tone: "red",
			},
			{
				label: "Stable core",
				value: "Mostly",
				detail: "Primary UI stack is aligned",
				tone: "green",
			},
		],
		...
		highlights: () => [
			{
				eyebrow: "Prioritize",
				title: "Not every update matters equally",
				body: "Rank upgrades by user impact, risk, and whether they touch trust-critical surfaces.",
				tone: "blue",
			},
			{
				eyebrow: "Explain",
				title: "Show why something is risky",
				body: "A major bump or vulnerability should include the affected lane, not just a version delta.",
				tone: "amber",
			},
			{
				eyebrow: "Bound",
				title: "Keep upgrades intentional",
				body: "Avoid making the dependency view feel like a call to churn every week.",
				tone: "violet",
			},
		],
		...
		activity: () => [
			{
				title: "React + Vite lane",
				meta: "Stable",
				detail: "Core UI stack looks healthy for the current surface pass",
				status: "ok",
			},
			{
				title: "Charting bundle weight",
				meta: "Watch",
				detail: "Keep dense-chart dependencies lazy where possible",
				status: "warn",
			},
			{
				...
				meta: "Soon",
				detail:
					"One transitive lane needs follow-up in the next maintenance pass",
				status: "critical",
			},
			{
				title: "Token packages",
				meta: "Aligned",
				detail: "Design system tarball versions appear consistent",
				status: "info",
			},
		],
		...
		tableRows: () => [
			{
				primary: "core UI stack",
				secondary: "Low",
				tertiary: "Keep aligned with current verified versions",
			},
			{
				primary: "charting",
				secondary: "Medium",
				tertiary: "Watch bundle growth and lazy-load strategy",
			},
			{
				primary: "security-sensitive transitive",
				secondary: "High",
				tertiary: "Schedule targeted update with validation",
			},
			{
				primary: "design-system vendor tarballs",
				secondary: "Medium",
				tertiary: "Refresh only when compatibility is clear",
			},
		],
		footerNote: () =>
			"Dependencies page should guide judgment, not generate upgrade anxiety.",
	},
	env: {
		section: "Health",
		title: "Env Files",
