...
import {
	...
	generateQueryId,
	hashQuestion,
} from "../../../core/narrative/causalRecall";
import {
...
export function useBranchAskWhyState(
	...
	const handleSubmitAskWhy = useCallback(
			...
			try {
				...
				if (isStaleResponse()) {
					trackAskWhyError({
						queryId,
						attemptId,
						errorType: "stale_ignored",
						branchScope,
						eventOutcome: "stale_ignored",
					});
					return;
				}
				...
				const startedAtForVersion =
					askWhyStartedAtByVersionRef.current.get(requestVersion);
				const elapsedMs =
					startedAtForVersion === undefined
						? undefined
						: (typeof performance !== "undefined"
								? performance.now()
								: Date.now()) - startedAtForVersion;
	...
	const handleOpenAskWhyCitation = useCallback(
