...
function deriveTrustState(status?: CaptureReliabilityStatus | null): DashboardTrustState {
...
export function RepoEvidenceOverview({
  ...
  const linkedSessions = model.sessionExcerpts?.filter((session) => Boolean(session.linkedCommitSha)).length ?? 0;
  const floatingSessions = (model.sessionExcerpts?.length ?? 0) - linkedSessions;
  const traceCommitCount = Object.keys(model.traceSummaries?.byCommit ?? {}).length;
  ...
  const evidenceMetrics: Array<{
  ...
  }> = [
    {
      ...
      detail: evidenceCount > 0 ? 'Narrative claims already point back to concrete evidence objects.' : 'No stable claim should survive without linked proof.',
    ...
    },
    {
      ...
      detail: floatingSessions > 0 ? `${floatingSessions} still floating.` : 'Current session evidence is already joined.',
    ...
    },
    {
      ...
      detail: traceCommitCount > 0 ? 'Visible commit lane already exposes trace summaries.' : 'No trace coverage on the visible commit lane yet.',
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1.5 text-[0.8125rem] leading-5 text-text-secondary">{action.body}</p>
                  </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {evidenceMetrics.map((metric) => (
          <article key={metric.label} className="rounded-[1.1rem] border border-border-subtle bg-bg-primary/80 p-3.5">
            ...
            <p className="mt-2.5 text-[1.7rem] font-semibold tracking-tight text-text-primary">{metric.value}</p>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[1.1rem] border border-border-light bg-bg-primary/70 px-3.5 py-2.5 text-sm text-text-secondary">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-text-primary">
            {model.narrative?.summary ?? model.description ?? 'The branch narrative is still being assembled from repo evidence.'}
          </span>
        </div>
