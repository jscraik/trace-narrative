		...
		<div className="border-b border-border-subtle py-3 last:border-0">
			<div className="flex items-start gap-3">
				...
				<div className="flex-1 min-w-0">
					...
					{test.errorMessage && (
						<div className="mt-1 font-mono text-xs text-accent-red">
							{test.errorMessage}
						</div>
					)}
					{filePath ? (
...
export function TestResultsPanel({
	...
	const existsMap = useRepoFileExistence(
		repoRoot ?? "",
		mentionedFiles.filter((p) => isRepoRelativePath(p)),
	);
			...
			<div className={`card p-5 ${className || ""}`}>
				...
				<div className="mt-5 flex flex-col items-center text-center py-3">
					...
					<p className="text-sm text-text-tertiary mb-1">
						{selectedCommitSha
							? "No test results for this commit yet"
							: "Select a commit to view tests"}
					</p>
					<p className="text-xs text-text-muted">
						Narrative doesn’t fetch CI automatically. Import a JUnit XML file to
						attach results here.
					</p>
			...
			{expanded && (
				<div id={panelId} className="border-t border-border-subtle px-5 pb-5">
					<div className="mt-4 text-[0.6875rem] text-text-muted">
						...
						{testRun.importedAtISO
							? ` · ${new Date(testRun.importedAtISO).toLocaleString()}`
							: ""}
					</div>
					...
					{mentionedFiles.length > 0 ? (
						<div className="mt-4">
							...
							<div className="mt-1 text-[0.6875rem] text-text-muted">
								From imported test results. Best-effort — may not be changed in
								this commit.
							</div>
