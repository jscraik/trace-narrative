...
export function UpdatePrompt({
	...
	function getUserFriendlyError(error: string): {
		...
		if (
			lowerError.includes("cannot read properties of undefined") &&
			lowerError.includes("invoke")
		) {
			return {
				title: "Desktop Features Unavailable",
				message:
					"Auto-updates are only available in the desktop app. Please download the latest version from our releases page.",
				isWebMode: true,
			};
		}
		...
		if (
			...
			return {
				title: "Server Error",
				message:
					"Our update server is temporarily unavailable. Please try again later.",
			};
		}
					...
					<div className="flex items-start gap-3">
						...
						<div className="flex-1">
							...
							<p className="text-xs text-text-tertiary mt-1">
								The update has been downloaded. Restart the app to apply
								changes.
							</p>
						</div>
						{handleClose ? (
...
export function UpdateIndicator({ status, onClick }: UpdateIndicatorProps) {
