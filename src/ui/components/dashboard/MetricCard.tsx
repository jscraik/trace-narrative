	...
	return (
		...
		>
			...
			<div className="flex items-baseline gap-3">
				<div
					className={`text-3xl font-bold tracking-tight ${valueColorClass ?? "text-text-primary"}`}
				>
					{value}
				</div>
			...
			{trendSubtitle && (
				<div className={`text-xs mt-1 ${trendColorClass}`}>
					...
					{trend?.icon === "trending_down" && (
						<TrendingDown className="w-3 h-3 inline mr-1" aria-hidden="true" />
					)}
					{trend?.icon === "minus" && (
						<Minus className="w-3 h-3 inline mr-1" aria-hidden="true" />
					)}
					{trendSubtitle}
				</div>
			)}
