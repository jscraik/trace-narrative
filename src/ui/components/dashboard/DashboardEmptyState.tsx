...
const MOCK_SIGNALS = [
	{
		...
		value: "142",
		tone: "blue",
		detail: "+12 vs previous window",
	},
	{
		label: "Attributed lines",
		value: "12.4K",
		tone: "violet",
		detail: "84% AI-linked evidence.",
	},
	{
		label: "Top tool lane",
		value: "Codex Edit",
		tone: "green",
		detail: "8,420 lines in the current window.",
	},
	{
		label: "Trust posture",
		value: "Ready",
		tone: "green",
		detail: "Capture is healthy.",
	},
] as const;
...
const EMPTY_STATES: Record<
	Exclude<DashboardEmptyReason, "no-repo">,
	EmptyStateConfig
> = {
	"no-commits": {
		...
		message:
			"There are no commits in the selected time period. Try a broader window so the narrative brief has evidence to work with.",
		primaryAction: {
			label: 'Select "All Time"',
			onClick: () => {
				/* Set time range to 'all' */
			},
		},
	...
	"no-ai": {
		...
		message:
			"Import Codex sessions first to establish a trustworthy baseline. Additional providers can be layered in after the Codex flow is stable.",
		primaryAction: {
	...
	"no-attribution": {
		...
		message:
			"Sessions exist but couldn't be linked to commits. Check your linking settings or use Repo Evidence to verify the joins manually.",
		primaryAction: {
						...
						<section className="grid gap-4 xl:grid-cols-[0.75fr_1.25fr]">
							...
							<article className="rounded-3xl border border-border-subtle bg-bg-secondary p-4">
								<div className="mb-3 flex items-center justify-between gap-3">
									<div>
										<p className="text-[0.6875rem] font-semibold uppercase tracking-[0.18em]">
											Change pulse
										</p>
						...
						<section className="grid gap-4 xl:grid-cols-[1fr_1fr]">
							<article className="rounded-3xl border border-border-subtle bg-bg-secondary p-4">
								<div className="mb-3">
									...
									<h3 className="mt-0.5 text-sm font-semibold">
										Line attribution by tool · top 3
									</h3>
								</div>
								<MiniBarChart
									data={MOCK_TOOLS}
									...
									<div className="flex w-full items-start gap-3 rounded-xl border border-border-light bg-bg-primary p-3">
										...
										<div>
											...
											<p className="mt-0.5 text-xs text-text-secondary">
												Start with the highest-signal file.
											</p>
										</div>
									</div>
									<div className="flex w-full items-start gap-3 rounded-xl border border-border-light bg-bg-primary p-3">
										...
										<div>
											<p className="text-sm font-semibold">
												Resolve trust posture
											</p>
											<p className="mt-0.5 text-xs text-text-secondary">
												Trust Center stays green.
											</p>
										</div>
									</div>
								</div>
