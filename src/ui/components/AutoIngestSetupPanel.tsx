...
export function AutoIngestSetupPanel(props: {
  ...
  const applyWatchPaths = useCallback((paths: WatchPaths, persist = true) => {
  ...
  const appendPath = async (current: string, onChange: (value: string) => void) => {
    ...
    <div className="card p-5">
      ...
      <div className="mt-4 space-y-4">
        <CaptureModeCard
          ...
          onAuthorize={() => runAuthAction(onAuthorizeCodexAppServerForLiveTest)}
        ...
        <CollectorMigrationCard
          ...
          onMigrateNow={() => (onMigrateCollector ? runMigrationAction(() => onMigrateCollector(false)) : Promise.resolve())}
          onDryRun={() => (onMigrateCollector ? runMigrationAction(() => onMigrateCollector(true)) : Promise.resolve())}
          onRollback={() => (onRollbackCollector ? runMigrationAction(onRollbackCollector) : Promise.resolve())}
        />

        <AutoIngestToggleCard enabled={config.autoIngestEnabled} onToggle={onToggleAutoIngest} />
