...
function confidenceBandStyle(
	band: AskWhyAnswerPayload["confidenceBand"],
): string {
...
function CitationButton({
	...
	return (
		...
		>
			...
			<span className="ml-1.5 uppercase tracking-wide text-text-muted">
...
export function AskWhyAnswerCard({
	state,
	onSubmit,
	onOpenCitation,
	onOpenRawDiff,
	disabled,
}: AskWhyAnswerCardProps) {
	...
	return (
		<div className="rounded-lg border border-border-subtle bg-bg-primary p-3">
			...
			{state.kind === "error" && (
				<div
					className="mt-2 rounded-md border border-accent-red-light bg-accent-red-bg px-2.5 py-2 text-xs text-accent-red"
					aria-live="polite"
				>
			...
			{state.kind === "ready" && state.answer && (
				<div className="mt-3 space-y-2" aria-live="polite">
					<div className="flex items-center justify-between gap-2">
						...
						<span
							className={`text-[0.6875rem] font-medium uppercase ${confidenceBandStyle(state.answer.confidenceBand)}`}
						>
							{state.answer.confidenceBand}{" "}
							{(state.answer.confidence * 100).toFixed(0)}%
						</span>
					</div>

					<p className="text-sm leading-relaxed text-text-primary">
						{state.answer.answerParagraph}
					</p>
					...
					{state.answer.fallbackUsed && (
						<div className="rounded-md border border-accent-amber-light bg-accent-amber-bg px-2.5 py-2 text-xs text-accent-amber">
							<p>
								Low confidence in this answer. Consider reviewing raw evidence
								directly.
							</p>
							<button
								type="button"
