...
describe('TopNav', () => {
  it('renders a calmer shell header with current section and contextual quick routes', () => {
    ...
    expect(screen.getByRole('button', { name: 'Repo Evidence' })).toBeInTheDocument();
    expect(screen.getByText('/Users/jamiecraik/dev/trace-narrative')).toBeInTheDocument();
  ...
  it('uses a single back-to-brief shortcut on the secondary docs lane', () => {
    ...
    expect(screen.getByRole('button', { name: 'Back to Brief' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Repo Evidence' })).not.toBeInTheDocument();
  ...
  it('keeps assistant compatibility paths folded into the Narrative Brief label', () => {
    ...
    expect(screen.getByText('Narrative')).toBeInTheDocument();
    expect(screen.getByText('Codex-guided asks now live inside stronger evidence views.')).toBeInTheDocument();
    expect(screen.queryByText('Codex Copilot')).not.toBeInTheDocument();
  });
});
