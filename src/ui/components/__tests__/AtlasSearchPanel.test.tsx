...
describe("AtlasSearchPanel", () => {
	...
	it("ignores stale rebuild completion after repository switch", async () => {
		...
		mockAtlasIntrospect.mockImplementation(async (repoId: number) =>
			ok(createIntrospect(repoId)),
		);
		mockAtlasDoctorReport.mockImplementation(async (repoId: number) =>
			ok(createDoctorReport(repoId)),
		);
