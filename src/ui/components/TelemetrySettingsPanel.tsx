...
export function TelemetrySettingsPanel({
	...
	const [otelPath, setOtelPath] = useState(
		traceConfig?.codexOtelLogPath ?? "/tmp/codex-otel.json",
	);
	...
	const appServerStreamActive =
		captureReliabilityStatus?.streamExpected ?? false;
	const showFilePathConfig =
		(!embeddedReceiverAvailable || !receiverEnabled) && !appServerStreamActive;
	const filePathHiddenReason = appServerStreamActive
		? {
				icon: "🛰",
				badge: "App Server stream active",
				message:
					"Codex App Server stream is active. File-path OTEL import is hidden.",
				badgeClass:
					"border-accent-violet-light bg-accent-violet-bg text-accent-violet",
	...
	const maskedKey =
		otlpKeyStatus?.maskedPreview ?? (keyPresent ? "********" : null);
		...
		return (
			<div className="card p-5">
				...
				<div className="mt-4 text-xs text-text-tertiary">
					Open a repo to configure telemetry settings.
				</div>
			</div>
		);
	}
			...
			<div className="mt-4 flex flex-col gap-4">
				...
				<div className="rounded-lg border border-border-subtle bg-bg-secondary p-3">
					...
					<div className="mb-3 text-[0.6875rem] text-text-tertiary">
						Uses a local OTLP receiver with an API key stored securely on this
						machine.
					</div>
					...
					{!hasConsent ? (
						<div className="mt-2 flex items-center justify-between gap-2 rounded-md bg-bg-tertiary p-2 border border-border-subtle">
							...
							<Toggle
								checked={false}
								onCheckedChange={(_c) => onGrantConsent?.()}
								aria-label="Grant Codex telemetry consent"
							/>
						</div>
					) : (
					...
					{showFilePathConfig ? (
						<div>
							...
							<div className="flex flex-wrap items-center gap-2">
								...
								<button
									type="button"
									className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-xs font-semibold transition duration-200 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] disabled:cursor-not-allowed disabled:opacity-50 ${
										otelPath.trim() &&
										otelPath !==
											(traceConfig?.codexOtelLogPath ?? "/tmp/codex-otel.json")
						...
						<div className="rounded-md border border-border-subtle bg-bg-secondary px-3 py-2 text-[0.6875rem] text-text-tertiary">
							<span
								className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[0.625rem] font-semibold ${filePathHiddenReason.badgeClass}`}
							>
								<span aria-hidden="true">{filePathHiddenReason.icon}</span>
								<span
									className={`inline-block h-1.5 w-1.5 rounded-full ${filePathHiddenReason.dotClass}`}
									aria-hidden="true"
								/>
						...
						<div className="flex items-center justify-between pt-2 mt-3 border-t border-border-subtle/50">
							<div className="flex flex-col gap-0.5">
								<span className="text-xs font-medium text-text-secondary">
									Embedded OTel receiver
								</span>
								<span className="text-[0.625rem] text-text-tertiary">
									Runs a local OTLP receiver (port 4318)
								</span>
							</div>
							<Toggle
								checked={receiverEnabled}
								onCheckedChange={(c) => onToggleCodexOtelReceiver(c)}
								aria-label="Embedded Codex OTel receiver (local)"
							/>
						</div>
					) : null}
