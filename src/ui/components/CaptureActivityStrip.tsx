...
function CaptureLifecycleRail({
	...
	const hasLinkedCommit = Boolean(
		latestImport?.commitShas && latestImport.commitShas.length > 0,
	);
				...
				{steps.map((step, index) => {
					...
					return (
						<div key={step.id} className="flex items-center gap-1.5">
							...
							<span className={`text-[0.625rem] leading-4 ${textClass}`}>
								{step.label}
							</span>
			...
			<div className="mt-1.5 flex flex-wrap gap-1.5">
				{[
					...
					<button
						...
						className={`rounded-md border px-2 py-0.5 text-[0.625rem] transition duration-200 ease-out active:duration-75 active:scale-[0.98] hover:scale-105 ${
...
export function CaptureActivityStrip(props: {
	...
	const filteredItems = useMemo(() => {
		...
		return items.filter((e) => {
			...
			if (activityFilter === "linked")
				return Boolean(e.commitShas && e.commitShas.length > 0);
			return true;
		});
					...
					<div>
						...
						{captureMode ? (
							<div className="mt-1 flex items-center gap-2">
								<span
									className={`inline-flex items-center rounded-full px-2 py-0.5 text-[0.625rem] font-semibold border ${
										captureMode === "HYBRID_ACTIVE"
				...
				<div className="pt-1">
					...
					{recent.length === 0 ? (
						<div className="mt-2 text-xs text-text-tertiary">
							No recent capture activity yet.
						</div>
					) : (
