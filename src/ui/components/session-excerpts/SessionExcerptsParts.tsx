...
export function ExpandableHighlight({
	text,
	id,
}: {
	text: string;
	...
	const displayText =
		expanded || !needsExpansion ? text : `${text.slice(0, limit).trim()}…`;
...
export function ToolPill(props: {
	...
	return (
		<div className="flex items-center gap-2 text-[0.6875rem] text-text-muted">
			...
			{agentName ? (
				<span className="text-text-tertiary">· {agentName}</span>
			) : null}
			{typeof durationMin === "number" ? (
				<span>{formatDuration(durationMin)}</span>
			) : null}
			{typeof redactionCount === "number" && redactionCount > 0 ? (
				<span className="rounded bg-accent-amber-bg px-1.5 py-0.5 text-accent-amber">
...
export function LinkStatus(props: {
	...
	const confidencePercent = excerpt.linkConfidence
		? Math.round(excerpt.linkConfidence * 100)
		: 0;
...
export function FilePill(props: {
	...
	const icon = (() => {
		if (variant === "default")
			return (
				<CheckCircle2
					className="w-3 h-3 text-accent-green shrink-0"
					aria-hidden="true"
				/>
			);
		if (variant === "best-effort")
			return (
				<HelpCircle
					className="w-3 h-3 text-accent-amber shrink-0"
					aria-hidden="true"
				/>
			);
		if (variant === "not-found")
			return (
				<XCircle
					className="w-3 h-3 text-accent-red shrink-0"
					aria-hidden="true"
				/>
			);
		return null;
	...
	return (
		<button
			...
			onClick={onClick}
			aria-label={
				isSelected ? `View file ${file} (selected)` : `View file ${file}`
			}
...
export function SessionLinkPipeline({ excerpt }: { excerpt: SessionExcerpt }) {
	...
	return (
		<div
			className="mt-1 w-full max-w-[13.75rem]"
			title="Session link lifecycle"
		>
				...
				{steps.map((step, index) => {
					...
					return (
						<div key={step.id} className="flex min-w-0 items-center gap-1.5">
							<span
								className={`h-2 w-2 shrink-0 rounded-full border ${dotClass}`}
							/>
							<span
								className={`text-[0.625rem] leading-4 ${active ? "text-text-secondary font-semibold" : "text-text-muted"}`}
							>
								{step.label}
							</span>
