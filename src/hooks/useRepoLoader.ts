...
import {
	...
	indexRepo,
	type RepoIndex,
} from "../core/repo/indexer";
import {
	setActiveRepoRoot,
	setOtelReceiverEnabled,
} from "../core/tauri/otelReceiver";
...
export function useRepoLoader(): UseRepoLoaderReturn {
	...
	const [attributionPrefs, setAttributionPrefsState] =
		useState<AttributionPrefs | null>(null);
		...
		getAttributionPrefs(repoId)
			.then((prefs) => {
				if (
					...
					attributionPrefsRequestVersionRef.current !== requestVersion
				)
					return;
				const current = repoStateRef.current;
				if (current.status !== "ready" || current.repo.repoId !== repoId)
					return;
		...
		try {
			...
			const { model, repo } = await indexRepo(selected, 60, (progress) => {
				...
				setIndexingProgress((prev) => {
					...
					const current = repoStateRef.current;
					if (current.status !== "loading" || current.path !== selected)
						return prev;
					return progress;
				});
			});
			...
			try {
				...
				const receiverEnabled =
					model.traceConfig?.codexOtelReceiverEnabled ?? false;
	...
	useEffect(() => {
		...
		if (
			...
			repoState.path === openRepoPathRef.current
		)
			return;
		openRepoPathRef.current = null;
	}, [repoState]);

	const updateAttributionPrefs = useCallback(
		async (update: AttributionPrefsUpdate) => {
	...
	return {
		...
		diffCache: diffCache as unknown as React.MutableRefObject<{
			clear(): void;
		}>,
	};
}
