...
export function useAtlasSearch(
	repoId: number | null,
	opts?: { limit?: number; debounceMs?: number; maxSessionChunks?: number },
): UseAtlasSearchState {
	...
	const [selectedSession, setSelectedSession] =
		useState<AtlasGetSessionResponse | null>(null);
	...
	useEffect(() => {
		...
		const handle = window.setTimeout(() => {
			(async () => {
				...
				const nextResults = Array.isArray(env.value.results)
					? env.value.results
					: [];
