...
function testBadgeForRun(run: { passed: number; failed: number }): {
	label: string;
	status: "passed" | "failed";
} {
	if (run.failed > 0)
		return { label: `${run.failed} failed`, status: "failed" };
	...
	const importJUnitForCommit = useCallback(
		async (commitSha: string) => {
			...
			const isStaleRequest = () =>
				!isActiveRequest(expectedRepoId, expectedRepoRoot, requestVersion);
			...
			try {
				...
				if (!isActiveRequest(expectedRepoId, expectedRepoRoot, requestVersion))
					return;
