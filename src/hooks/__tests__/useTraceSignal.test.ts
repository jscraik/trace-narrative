...
function makeSummary(
	overrides: Partial<TraceCommitSummary> = {},
): TraceCommitSummary {
	return {
		commitSha: "c1",
		aiLines: 10,
...
function makeOptions(
	overrides: Partial<UseTraceSignalOptions> = {},
): UseTraceSignalOptions {
	return {
		selectedNodeId: null,
...
describe("useTraceSignal", () => {
	...
	it("enforces matrix + precedence (Idle cannot jump directly to Insight)", async () => {
		const { result, rerender } = renderHook(
			(options: UseTraceSignalOptions) => useTraceSignal(options),
			{
	...
	it("dedupes Insight by key and supports Insight→Insight only when key changes", async () => {
		...
		expect(result.current.event.type).toBe("insight");
		const firstKey =
			result.current.event.type === "insight"
				? result.current.event.insightKey
				: "";
