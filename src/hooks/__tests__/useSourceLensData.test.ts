...
describe('useSourceLensData', () => {
  beforeEach(() => {
    ...
    mockImportAttributionNote.mockResolvedValue({ status: 'ok', importedRanges: 1 });
    mockExportAttributionNote.mockResolvedValue({ status: 'ok', commitSha: 'default' });
    mockSetAttributionPrefs.mockResolvedValue(createPrefs(1));
  });

  it('ignores stale source-lens payloads after commit change', async () => {
    const staleLens = createDeferred<{ lines: Array<{ lineNumber: number; content: string; authorType: 'human' }>; totalLines: number; hasMore: boolean }>();
    ...
    mockInvoke.mockImplementation((_command: string, args: { request: { commitSha: string } }) => {
      ...
      return Promise.resolve({
        lines: [{ lineNumber: 1, content: 'new-line', authorType: 'human' as const }],
        totalLines: 1,
        hasMore: false,
      });
    });
  ...
  it('ignores stale contribution stats after commit change', async () => {
    ...
    mockGetCommitContributionStats.mockImplementation((repoId: number, commitSha: string) => {
