...
describe("useAutoIngest listener cleanup", () => {
	...
	it("stops watcher even when startFileWatcher resolves after cleanup", async () => {
		...
		mockStartFileWatcher.mockImplementationOnce(
			async () => startDeferred.promise,
		);
	...
	it("skips late bootstrap side effects after unmounting during initial load", async () => {
		...
		mockDiscoverCaptureSources.mockImplementation(
			() => sourcesDeferred.promise,
		);
		mockGetCollectorMigrationStatus.mockImplementation(
			() => migrationDeferred.promise,
		);
		mockGetCaptureReliabilityStatus.mockImplementation(
			() => reliabilityDeferred.promise,
		);
		mockGetCodexAppServerStatus.mockImplementation(
			() => appServerDeferred.promise,
		);
	...
	it("handles all session:live:event payload variants in a synthetic sequence", async () => {
		...
		await waitFor(() => {
			expect(mockGetCaptureReliabilityStatus.mock.calls.length).toBeGreaterThan(
				reliabilityCalls,
			);
		});
	...
	it("does not refresh badges or activity after unmounting during listener-triggered auto-import", async () => {
		...
		mockListen.mockImplementation(
			async (
				_event: string,
				_handler: (event: SessionEvent) => Promise<void> | void,
			) => {
		...
		let sessionCallback:
			| ((event: SessionEvent) => Promise<void> | void)
			| undefined;
		await waitFor(() => {
		...
		expect(mockAutoImportSessionFile).toHaveBeenCalledWith(
			1,
			"/tmp/session.json",
		);
