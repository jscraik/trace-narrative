...
describe("useTraceCollector listener cleanup", () => {
	...
	it("ignores ingest updates when current model repoId does not match hook repoId", async () => {
		...
		expect(mockScanAgentTraceRecords).toHaveBeenCalledWith("/repo/1", 1, [
			"c1",
		]);
	...
	it("ignores stale action errors after repository context changes", async () => {
		...
		mockSaveTraceConfig.mockImplementationOnce(
			async () => saveDeferred.promise,
		);
		...
		await waitFor(() => {
			expect(mockSaveTraceConfig).toHaveBeenCalledWith(
				"/repo/1",
				expect.any(Object),
			);
		});
