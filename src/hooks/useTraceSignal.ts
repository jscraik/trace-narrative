...
export const TRACE_PRECEDENCE = ['insight', 'analyzing', 'tracking', 'idle'] as const;
export type TraceStateType = (typeof TRACE_PRECEDENCE)[number];
export type TraceLoader = 'files' | 'diff' | 'trace';

export const TRACE_TRANSITION_MATRIX: Record<TraceStateType, readonly TraceStateType[]> = {
...
export type TraceEvent =
  ...
  | { type: 'analyzing'; selectedNodeId: string; pendingLoaders: TraceLoader[]; pendingKey: string }
...
function getPendingApplicableLoaders(options: UseTraceSignalOptions): TraceLoader[] {
...
function sameEvent(a: TraceEvent, b: TraceEvent): boolean {
  ...
  switch (a.type) {
    case 'idle':
      return a.selectedNodeId === (b as Extract<TraceEvent, { type: 'idle' }>).selectedNodeId;
    case 'tracking':
      return a.selectedNodeId === (b as Extract<TraceEvent, { type: 'tracking' }>).selectedNodeId;
    case 'analyzing': {
 ...
 */
export function useTraceSignal(options: UseTraceSignalOptions): UseTraceSignalReturn {
  const [enabled, setEnabled] = useState(true);
  const [loading, setLoading] = useState(true);
  const [event, setEvent] = useState<TraceEvent>({ type: 'idle', selectedNodeId: null });
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        console.error('[trace.settings.load_failed]', error);
        if (!cancelled) {
          options.onPersistenceError?.(`Unable to load Trace settings: ${getErrorMessage(error)}`);
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  const toggle = useCallback(async (nextEnabled?: boolean) => {
    ...
    try {
      ...
      options.onPersistenceError?.(`Unable to persist Trace setting: ${message}`);
    }
  }, [options]);
  ...
  useEffect(() => {
      ...
      switch (targetType) {
        case 'tracking':
          return options.selectedNodeId
            ? ({ type: 'tracking', selectedNodeId: options.selectedNodeId } satisfies TraceEvent)
        ...
        case 'insight':
          return options.selectedNodeId && options.selectedCommitSha && insightKey
            ...
            : ({ type: 'idle', selectedNodeId: null } satisfies TraceEvent);
        default:
          return { type: 'idle', selectedNodeId: options.selectedNodeId } satisfies TraceEvent;
      }
    })();
    ...
    if (!selectionChanged) {
      if (currentEvent.type === 'analyzing' && nextEvent.type !== 'analyzing' && nextEvent.type !== 'insight') {
        dwellRemaining = Math.max(0, TRACE_ANALYZING_DWELL_MS - (now - stateEnteredAtRef.current));
      }
      ...
      if (currentEvent.type === 'insight' && nextEvent.type !== 'insight') {
        ...
        if (!analyzingRestart) {
          dwellRemaining = Math.max(dwellRemaining, TRACE_INSIGHT_DWELL_MS - (now - stateEnteredAtRef.current));
        }
      }
    }
